...
void Date::validate(int day, int month, int year) {
    ...
    if ( month == 2 || month == 4 || month == 6 || month == 9 || month == 11 ) {
        if ( day == 31 ) {
            InvalidDate("This month doesn`t contain so many days!");
        }
    }
    if ( day == 30 && month == 2 ) {
        throw InvalidDate("February doesn`t have so days!");
    }
    if ( year % 4 != 0 ) {
        if ( day == 29 && month == 2 ) {
...
std::ostream& operator<<(std::ostream& out, const Date& date) {
    out << "(" << std::setfill('0') << std::setw(2) << date.getDay() << ":" << std::setw(2) << date.getMonth() << ":" << std::setw(4) << date.getYear() << ")" << std::endl;
